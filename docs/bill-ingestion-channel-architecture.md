...
`/bill-inbox` 是渠道工作台首页，不再按一封邮件一行展示任务。首页固定展示所有内置渠道，即使当前没有账单批次：
...
`/bill-inbox/channel/{source}` 是渠道详情页。这里才展示该渠道下的邮件批次、附件数量、流水数量、验证码输入、归档和状态筛选。默认隐藏已归档批次，用户通过状态筛选查看归档记录。

`/bill-inbox/{billTask}` 继续作为单个批次详情页，用于查看附件、事件、流水明细和导入操作。
...
中国银行交易流水渠道第一版只覆盖邮箱识别、加密 PDF 保存和验证码挑战。PDF 解密、表格抽取和字段映射等看到真实样本后再做：

- 来源：`boc`
- 类型：`boc-transaction-statement`
- 邮件搜索：`SUBJECT "中国银行交易流水"`
- 邮件识别：主题包含 `中国银行交易流水`，正文包含 `中国银行APP` 或 `交易流水打印`，并带 PDF 附件
- 附件：邮件直接携带加密 PDF，保存为 `bill_artifacts.kind=pdf`，`encrypted=true`
- 验证码提示：`请输入中国银行APP“交易流水打印”申请记录中的打开密码`
- 密码来源：中国银行 APP 的交易流水打印申请记录
- 提交密码后的第一版行为：不保存明文密码，不解密 PDF，不解析流水；任务标记为已解析，metadata 写入 `parser_status=waiting_for_pdf_mapping`
- 后续解析要求：拿到真实 PDF 后确认文本结构、表格结构、金额方向、对方账户、摘要、币种和流水号，再新增中国银行专用 PDF import service，把结果写入导入中间表

中国银行渠道不得复用其它渠道的 ZIP 解压或 CSV/XLSX 字段映射。PDF 可能存在页面文本、表格坐标和字体编码差异，必须用真实样本驱动解析器设计。
